...
export default function Home() {
    ...
    // Dashboard View: The grid of cards
    if (activeEntryId === null) {
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 sm:p-12 animate-in fade-in zoom-in-95 duration-700">
                <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-8">
                    {/* User Card */}
                    <Card className="border-none shadow-2xl bg-white rounded-[40px] p-8 flex flex-col justify-between min-h-[340px] group">
                        <div className="space-y-6">
                            ...
                            <div>
                                <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{user?.account || "管理员"}</h3>
                                <p className="text-slate-400 font-medium text-lg">{user?.email || "admin@pism.com"}</p>
                            </div>
                        </div>
                        <Button
                            variant="ghost"
                    ...
                    {entries.map((entry) => (
                        ...
                        >
                            ...
                            <CardDescription className="text-slate-400 text-xl font-medium leading-tight">{entry.description}</CardDescription>
    ...
    // Sidebar Shell View: Minimalist vertical navigation
    return (
        <div className="flex h-screen bg-white overflow-hidden animate-in fade-in duration-500">
            {/* Sidebar Navigation */}
            <aside className="w-24 bg-slate-50 border-r border-slate-100 flex flex-col items-center py-10 z-30">
                {/* Identity At Top */}
                ...
                <nav className="flex-1 flex flex-col gap-6">
                    {entries.map((entry) => (
                        <button
                            key={entry.id}
                            onClick={() => setActiveEntryId(entry.id)}
                            className={`p-4 rounded-2xl transition-all duration-300 relative group ${activeEntryId === entry.id
                        ...
                        >
                            ...
                            {activeEntryId === entry.id && (
                                <span className="absolute -right-3 top-1/2 -translate-y-1/2 w-1.5 h-6 bg-primary rounded-full" />
                            )}
                        </button>
                    ))}
                </nav>
                ...
                <div className="mt-auto flex flex-col gap-6">
                    <button
                        ...
                        className="p-4 rounded-2xl bg-white text-slate-400 hover:text-primary hover:shadow-md transition-all duration-300"
                        title="返回首页"
                    >
                    ...
                    <button
                        onClick={logout}
                        className="p-4 rounded-2xl bg-white text-slate-400 hover:text-red-600 hover:shadow-md transition-all duration-300"
                        title="退出登录"
                    >
            ...
            {/* Main Workspace Area (Fully Immersive) */}
                    ...
                    <div className="w-full min-h-[500px] border-4 border-dashed border-slate-50 rounded-[60px] flex items-center justify-center animate-in fade-in slide-in-from-bottom-8 duration-1000">
                        <div className="text-center space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-slate-200 text-xl font-bold">Pism Phoenix Core Framework Integration Required</p>
                            </div>
                        </div>
                    </div>
                </div>
